...
int testaOrdem(LISTA *pLista);
int buscaR(LISTA *pLista, int numero);
...
//		clrscr();
		...
		if(op == 7){//busca numero
			...
			if(buscaR(inicio, numero) == 0){
				cout<<"\nLista esta vazia ou numero nao encontrado.";
			}else{
				cout<<"\nNumero encontrado.";
			}
		}
...
int testaOrdem(LISTA *pLista){
	int testa;
	if(pLista == NULL || pLista->num > pLista->prox->num){
		return 1; //fora da ordem crescente
	}else{
		return testaOrdem(pLista->prox);//dentro da ordem crescente
	}
}

int buscaR(LISTA *pLista, int numero){
	int i;
	if(pLista->num == numero){
		return 1; 
	}else{
		return buscaR(pLista->prox, numero);
	}
}
