...
int main(void){
	...
	do{
		...
		if(op == 1){
			cout<<"Digite o n�mero a ser inserido no inicio: ";
			LISTA *novo = new LISTA();
			cin>>novo->num;
			
			if(inicio == NULL){
				//lista vazia o elemento ser� inserido no in�ncio
				inicio = novo;
				fim = novo;
				fim->prox = inicio;
			}else{
					...
					if(inicio == fim && inicio->num == numero){
						...
						if(aux->num == numero){
							...
							if(aux == inicio){
								...
								//o numero foi encontrado na �ltima posi��o
								fim = anterior;
								fim->prox = inicio;
								delete(aux);
								aux = NULL;
